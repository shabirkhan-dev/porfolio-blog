  ...
  if (featured) {
    return (
      <Link href={`/blog/${post.slug}`} className="group block">
        <article className="relative grid overflow-hidden rounded-lg border border-border bg-background transition-colors duration-300 hover:border-border-strong hover:bg-card lg:grid-cols-[1.4fr_1fr]">
          ...
          <div className="relative p-8 sm:p-10">
            ...
            <h2 className="mt-8 max-w-2xl font-display text-[clamp(1.45rem,1.15rem+1.1vw,2.1rem)] font-semibold leading-[1.08] tracking-tight transition-colors group-hover:text-accent">
              {post.title}
            </h2>
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="group block h-full">
      <article className="relative flex h-full flex-col overflow-hidden rounded-lg border border-border bg-background p-7 transition-all duration-300 hover:-translate-y-1 hover:border-border-strong hover:bg-card hover:shadow-[0_18px_40px_-24px_rgba(0,0,0,0.5)]">
        ...
        <h2 className="mt-8 font-display text-xl font-semibold leading-[1.12] tracking-tight transition-colors group-hover:text-accent">
          {post.title}
        </h2>
